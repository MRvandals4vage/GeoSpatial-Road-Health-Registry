...
const MapView: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const pathLayer = new PathLayer<Road>({
            ...
            getColor: (d: Road) =>
                selectedRoad?.id === d.id ? CONDITION_HIGHLIGHT[d.condition] : CONDITION_COLOR[d.condition],
            widthMinPixels: 4,
            widthScale: 2,
            pickable: true,
            onHover: (info) => {
                ...
                if (info.object && el) {
                    ...
                    el.innerHTML = `<strong>${road.name}</strong><br/>Condition: ${road.condition}<br/>Score: ${road.conditionScore}`;
    ...
    }, [roads, selectedRoad, setSelected]);
